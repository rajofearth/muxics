...
export const HomeFeed = memo(function HomeFeed({ onNavigate }: { onNavigate?: (view: NavView, id?: string) => void }) {
  ...
  const handleItemClick = (item: Track | Playlist, sectionItems: (Track | Playlist)[]) => {
    if ("title" in item) {
      // It's a track
      const tracksOnly = sectionItems.filter((i): i is Track => "title" in i);
      playTrack(item, tracksOnly);
    } else if (onNavigate) {
      // It's a playlist or album. We use playlist_detail for both when browsing YT Music
      // because our playlist_detail view is better at hydrating remote collections by ID.
      onNavigate("playlist_detail", item.id);
    }
  };

  return (
    <div className="flex-1 overflow-y-auto pb-12 pt-4">
      {homeFeed.sections.map((section, idx) => (
        <section key={`${section.title}-${idx}`} className="mb-10 last:mb-0">
          ...
          <div className="flex overflow-x-auto gap-4 px-8 pb-4 scroll-smooth scrollbar-none">
            {section.items.map((item) => {
              ...
              return (
                <button
                  ...
                  className="group flex-shrink-0 w-[150px] text-left transition-all hover:scale-[1.02] active:scale-[0.98]"
                >
                  ...
                  <div className="text-[13px] font-semibold text-app-text-primary line-clamp-1 leading-tight mb-1 group-hover:text-app-text-primary/90">
                    {title}
                  </div>
                  <div className="text-[11.5px] text-app-text-tertiary line-clamp-1 group-hover:text-app-text-secondary">
                    {subtitle}
                  </div>
                </button>
              );
